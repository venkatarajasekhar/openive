...
int ncp_send(openive_info * vpninfo, char *buf, unsigned short len)
{
	...
	if (vpninfo->compression) {
		...
		unsigned short compr_len =
		    65534 - vpninfo->deflate_strm.avail_out;
...
{
	char hello[] = { 0x13, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x06, 0x00,
		'd', 'e', 'b', 'i', 'a', 'n', 0xbb, 0x01, 0x00, 0x00, 0x00, 0x00
	};
...
{
	char mtu[] = { 0x24, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x01, 0x2f, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x10, 0x00, 0x06, 0x00, 0x00, 0x00, 0x0a,
		0x00, 0x02, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x05, 0x78
	};
...
int make_ncp_connection(openive_info * vpninfo)
{
	...
	if (compression) {
		...
		deflateInit2(&vpninfo->deflate_strm, Z_DEFAULT_COMPRESSION,
			     Z_DEFLATED, -10, 8, Z_DEFAULT_STRATEGY);
